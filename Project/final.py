...
class BlackjackGame:
    ...
    def calculate_results(self):
        ...
        for player in self.players:
            ...
            if player_total > 21:
                ...
                if player.blackjack:
                    winnings = int(player.bet * 1.5)
                    player.balance += winnings
                    results.append(f"{player.name} wins Blackjack payout of ${winnings}!")
                else:
                    winnings = player.bet
                    player.balance += winnings * 2
        ...
        if not self.players:
            results.append("All players are out of money! Game over.")
            self.actions_frame.pack_forget()
            self.bet_frame.pack_forget()
    ...
    def update_player_labels(self):
        """Update player labels after removing players."""
        ...
        for player in self.players:
            ...
            label = tk.Label(
                self.players_frame,
                text=f"{player.name}: {hand_display} | Balance: ${player.balance} | Bet: ${player.bet}",
                font=("Helvetica", 16),
            )
            label.pack()
